...
It combines technical notes and project documentation in one searchable website. Instead of organizing knowledge only by course or publication date, Geshu organizes it around engineering capabilities: what problem something solves, how to build it, and how it fails in practice.
...
│   ├── index.md            # Homepage
│   ├── learn/              # Reading paths across capabilities
│   ├── notes/              # Technical notes, plus generated reference
...
│   └── overrides/          # Templates, partials, and the stylesheet
│
├── backend/                # Content store and future service code
│   ├── app/                # HTTP surface (not yet implemented)
│   ├── agents/             # Agent code (not yet implemented)
│   └── data/vaults/        # Private Obsidian vaults — never committed
│
├── scripts/
│   ├── sync_vault.py       # Vault -> docs/ converter
│   ├── vault_manifest.toml # Publication policy: what ships, what never does
│   ├── test_sync_vault.py  # Converter tests
│   └── githooks/           # Optional pre-commit privacy check
│
├── .github/workflows/      # Build and deployment automation
├── mkdocs.yml              # Site configuration and navigation
├── requirements.txt        # Python dependencies
├── .gitignore              # Keeps the vaults and build output out of git
├── LICENSE                 # MIT, for the site code
├── LICENSE-CONTENT         # CC BY-NC-SA 4.0, for the written notes
└── README.md
```

`backend/data/vaults/` holds the private Obsidian vaults the reference notes are
generated from. It is gitignored and exists only on the author's machine, so a
clone will contain the published pages but not the sources behind them.

Build output (`site/`) and the virtual environment (`.venv/`) are likewise
ignored — both are regenerated by the commands below.
...
Run synchronization tests. They are plain `unittest`, so they need nothing
...
Hand-written engineering notes and project pages remain separate from generated reference content.
...
* lab, build-log, and postmortem pages;

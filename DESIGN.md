...
§10 的目录结构（`vault/`、`app/`、`api/`）描述的是一个 JS 应用。**没有采用。**
前端继续用 MkDocs Material。

三样不该丢的东西决定了这个判断：57 篇已发布的笔记、一套手调过对比度的设计系统、
以及一条把私有 Obsidian vault 安全发布出去的转换管线
（`scripts/sync_vault.py` + `vault_manifest.toml`）。重写会把这三样一起扔掉，
换来的只是一个还没有内容的框架。

§8 要求的「页面调用自己的服务端 endpoint」并不需要重写前端：静态页面加一小段
JS，去 `fetch()` 一个后端即可。代价是 GitHub Pages 只能托管静态文件，所以后端
需要第二个部署目标——这一点连同它的取舍写在
[`backend/README.md`](backend/README.md)。

§10 的目录树保留在上面，作为被否决的方案的记录，不是待办事项。

---

*这份文档是设计意图。仓库当前实现到哪一步、怎么跑起来、怎么写一篇笔记，
